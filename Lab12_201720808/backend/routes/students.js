...
router.post("/", async (req, res) => {
  ...
  try {
    const { studentId, koreanName, englishName, department, photoUrl } =
      req.body;
    ...
    if (error.message.includes("duplicate")) {
      ...
      return res.status(500).send("학번은 숫자만 입력 가능하다");
    }
...
// delete /student/:studentId  특정 학번을 가진 학생의 데이터를 삭제하는 것.
